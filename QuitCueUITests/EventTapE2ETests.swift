...
@MainActor
final class EventTapE2ETests: QuitCueUITestCase {
    private static let stagedBundlePath = "/Applications/QuitCue.app"
    ...
    func testCmdQOnNonWhitelistedAppQuitsIt() throws {
        let cmdq = launchStagedQuitCue(whitelist: [])
    ...
    func testCmdQOnWhitelistedAppIsBlocked() throws {
        ...
        XCTAssertTrue(
            ...
            "Whitelist injection did not reach staged QuitCue.app"
        )

        let calc = launchCalculator()
        sendCmdQ()

        // Give QuitCue a beat to see + swallow the event.
        Thread.sleep(forTimeInterval: 1.0)
        XCTAssertTrue(
            isCalculatorRunning(),
            "Calculator should still be alive when QuitCue intercepts ⌘Q"
        )
    ...
    /// sequence may keep auto-repeating while focus changes. QuitCue must
    /// keep swallowing that sequence until key-up so only the protected target
    /// quits.
    func testHoldConfirmQuitsTargetButKeepsQuitCueRunning() throws {
        let cmdq = launchStagedQuitCue(
            whitelist: [Self.calculatorBundleID],
            mode: "hold",
            holdDuration: 0.2
        ...
        XCTAssertFalse(
            NSRunningApplication.runningApplications(
                withBundleIdentifier: "com.quitcue.QuitCue"
            ).isEmpty,
            "QuitCue must keep running after confirming a protected app quit"
        )
    }

    // MARK: - Helpers

    private func launchStagedQuitCue(
        whitelist: [String],
        mode: String = "hold",
        ...
        var args: [String] = [
            ...
            "-com.quitcue.confirmMode", mode,
            "-QuitCue.eventTapDiagnostics", "YES"
        ]
        if let holdDuration {
            args.append("-com.quitcue.holdDuration")
            args.append(String(holdDuration))
        }
        if showSettings {
            args.append("-QuitCue.showSettingsOnLaunch")
            args.append("YES")
        }
        if !whitelist.isEmpty {
            let plistArray = "(" + whitelist
                .map { "\"\($0)\"" }
                .joined(separator: ", ") + ")"
            args.append("-com.quitcue.whitelist.bundleIDs")
            args.append(plistArray)
        }
        app.launchArguments = args
        app.launch()
        // QuitCue needs a moment to install its event tap.
