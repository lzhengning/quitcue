...
final class OnboardingFlowUITests: QuitCueUITestCase {

    private func launch(onboardingComplete: Bool, startStep: Int? = nil) -> XCUIApplication {
        let app = XCUIApplication()
        var args = [
"-com.quitcue.onboarding.completed", onboardingComplete ? "YES" : "NO"
        ]
        if let startStep {
            args += ["-QuitCue.onboardingStartStep", String(startStep)]
        }
